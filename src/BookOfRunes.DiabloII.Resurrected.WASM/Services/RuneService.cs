...
namespace BookOfRunes.DiabloII.Resurrected.WASM.Services
{
	...
	public class RuneService(IRequestSender Sender, ILocalStorageService storage) : IRuneService
	{
		...
		private IEnumerable<Rune> _runes = Enumerable.Empty<Rune>().ToList();
		...
		public async Task LoadAsync(CancellationToken cancellationToken)
		{
			...
			_runes = response!.Select(r => new Rune { Id = r.Id, Selected = selected?.Any(id => r.Id == id) ?? false, Name = r.Name, Level = r.Level, InHelmet = r.InHelmet, InBodyArmor = r.InBodyArmor, InShield = r.InShield, InWeapon = r.InWeapon }).ToList();
