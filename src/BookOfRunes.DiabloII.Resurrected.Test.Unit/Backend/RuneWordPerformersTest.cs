...
namespace BookOfRunes.DiabloII.Resurrected.Test.Unit.Backend
{
	public class RuneWordPerformersTest : IDisposable
	{
		...
		public async Task RuneWordPerformers_GetRuneWordsQuery_FilterByItemType()
		{
			...
			// Act
			var result = await sut.PerformAsync(new GetRuneWordsQuery(new List<int> { runeWord.ItemTypes.First().Id }, 1, 6, 99), default);
		...
		public async Task RuneWordPerformers_GetRuneWordsQuery_ByLevel()
		{
			...
			// Act
			var result = await sut.PerformAsync(new GetRuneWordsQuery(runeWord.ItemTypeSwitch.Select(it => it.ItemType.Id).Concat(runeWordFiltered.ItemTypeSwitch.Select(it => it.ItemType.Id)), runeWord.RuneSwitch.Count, runeWord.RuneSwitch.Count + 1, 1), default);
