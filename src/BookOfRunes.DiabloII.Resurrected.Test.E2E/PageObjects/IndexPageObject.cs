...
namespace BookOfRunes.DiabloII.Resurrected.Test.E2E.PageObjects
{
	public class IndexPageObject
	{
		...
		{
			get => int.Parse(_page.GetByTestId("inpLevelFilter").InputValueAsync().GetAwaiter().GetResult()); set => _page.GetByTestId("inpLevelFilter").FillAsync(value.ToString()).GetAwaiter().GetResult();
		}
