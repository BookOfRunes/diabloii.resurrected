...
using RuneDex.DiabloII.Resurrected.WASM.Defaults;
...
namespace RuneDex.DiabloII.Resurrected.Test.Unit.WASM.Services
{
	public partial class CharacterServiceTest
	{
		...
		public async Task CharacterService_LoadAsync_GetCurrentCharacter()
		{
			...
			_storageMock.Setup(s => s.GetItemAsStringAsync("characters", It.IsAny<CancellationToken>()))
				.ReturnsAsync(JsonConvert.SerializeObject(characters.AsResponse(), new JsonSerializerSettings { Converters = [new StringEnumConverter()] }));
		...
		public async Task CharacterService_Next_SelectNextCharacter()
		{
			...
			_storageMock.Setup(s => s.GetItemAsStringAsync("characters", It.IsAny<CancellationToken>()))
				.ReturnsAsync(JsonConvert.SerializeObject(new List<Character> { new AutoFaker<Character>().Generate(), character }.AsResponse(), new JsonSerializerSettings { Converters = [new StringEnumConverter()] }));
		...
		public async Task CharacterService_Next_SelectNextCharacterAfterTheLast()
		{
			...
			_storageMock.Setup(s => s.GetItemAsStringAsync("characters", It.IsAny<CancellationToken>()))
				.ReturnsAsync(JsonConvert.SerializeObject(new List<Character> { character }.AsResponse(), new JsonSerializerSettings { Converters = [new StringEnumConverter()] })); ;
		...
		public async Task CharacterService_Previous_SelectPrevious()
		{
			...
			_storageMock.Setup(s => s.GetItemAsStringAsync("characters", It.IsAny<CancellationToken>()))
				.ReturnsAsync(JsonConvert.SerializeObject(new List<Character> { character, new AutoFaker<Character>().Generate() }.AsResponse(), new JsonSerializerSettings { Converters = [new StringEnumConverter()] }));
		...
		public async Task CharacterService_Previous_SelectNextRaisesChangedEvent()
		{
			...
			_storageMock.Setup(s => s.GetItemAsStringAsync("characters", It.IsAny<CancellationToken>()))
				.ReturnsAsync(JsonConvert.SerializeObject(new AutoFaker<Character>().Generate(2).AsResponse(), new JsonSerializerSettings { Converters = [new StringEnumConverter()] }));
		...
		public async Task CharacterService_SaveAsync_SetCurrentCharacterAfterAdd()
		{
			...
			_senderMock.Setup(s => s.SendAsync<GetItemTypesQuery, IEnumerable<GetItemTypesQuery.Result>>(It.IsAny<GetItemTypesQuery>(), It.IsAny<CancellationToken>()))
				.ReturnsAsync(GetItemTypes(character.Class));
		...
		public async Task CharacterService_SaveAsync_StoreAddedCharacter()
		{
			...
			// Assert
			_storageMock.Verify(s => s.SetItemAsync("characters", It.IsAny<IEnumerable>(), It.IsAny<CancellationToken>()), Times.Once());
		}
		...
		private IEnumerable<GetItemTypesQuery.Result> GetItemTypes(Class @class)
		{
			switch (@class)
			{
				case Class.Amazon:
					return ClassDefaults.Filters.Amazon.AsResult();
				case Class.Assassin:
					return ClassDefaults.Filters.Assassin.AsResult();
				case Class.Barbarian:
					return ClassDefaults.Filters.Barbarian.AsResult();
				case Class.Druid:
					return ClassDefaults.Filters.Druid.AsResult();
				case Class.Necromancer:
					return ClassDefaults.Filters.Necromancer.AsResult();
				case Class.Paladin:
					return ClassDefaults.Filters.Paladin.AsResult();
				case Class.Sorceress:
					return ClassDefaults.Filters.Sorceress.AsResult();
				default:
					return [];
			}
		}
	...
	internal static class CharacterServiceTestExtensions
	{
		...
		public static IEnumerable<Action<Character>> AsInspectors(this IEnumerable<Character> characters)
		{
			foreach (var character in characters)
			{
				yield return (c) =>
				{
					...
					Assert.Collection(c.Filters.ItemTypes, [.. character.Filters.ItemTypes.AsInspectors()]);
				};
			}
		}
		...
		public static IEnumerable<GetItemTypesQuery.Result> AsResult(this IEnumerable<int> types)
		{
			var faker = new Faker();
			return types.Select(t => new GetItemTypesQuery.Result { Id = t, Class = faker.PickRandom<ItemClass>(), Name = faker.Person.FirstName });
		}
	}
